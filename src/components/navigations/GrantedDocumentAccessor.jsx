...
const GrantedDocumentAccessor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const savedDocContent = urlParams.get('savedDocContent') || '';
      const savedDocName = urlParams.get('savedDocName') || ''; 
      const isDocumentOpen = urlParams.get('isDocumentOpen'); 

      if (isDocumentOpen === "true") {
        console.log("Document is open without permission check.");
        if (savedDocContent && savedDocName) {
          localStorage.setItem("editorDocName", decodeURIComponent(savedDocName));
          localStorage.setItem("editorDocContent", decodeURIComponent(savedDocContent));
          navigate("/editor"); 
        } else {
          console.error("Invalid parameters");
        }
